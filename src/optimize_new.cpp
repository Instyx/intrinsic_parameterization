...
Results optimize_single_new(Eigen::MatrixXd &V, Eigen::MatrixXi &F, EnergyType method, std::string dir, std::string mesh_name, const bool init_with_intrinsic, const bool priority_queue_flips){
  return optimize_single_new(V, F, method, dir, mesh_name, init_with_intrinsic, priority_queue_flips, false);
}

Results optimize_single_new(Eigen::MatrixXd &V, Eigen::MatrixXi &F, EnergyType method, std::string dir, std::string mesh_name, const bool init_with_intrinsic, const bool priority_queue_flips, const bool rando){
  ...
  while(itr<max_iterations && std::abs(past_energy-curr_energy)>tol){
    ...
    if (priority_queue_flips) {
      ...
      total_flips = queue_flip(data_mesh, UV_iparam, total_del_flips, method, rando);
    }
    end = std::chrono::high_resolution_clock::now();
...
// tries random flipping order multiple times; 
// also with priority queue and qeueue orders to compare
Results optimize_single_random(Eigen::MatrixXd &V, Eigen::MatrixXi &F, EnergyType method, std::string dir, std::string mesh_name, const bool init_with_intrinsic, unsigned random_runs){
  ...
  UV_iparam = UV_ext;
  while(itr<max_iterations && std::abs(past_energy-curr_energy)>tol){
    std::cout << "Iteration pq: " << itr+1 << std::endl;
    past_energy = curr_energy;
    //intrinsic flipping
    unsigned total_flips = 0;
    unsigned total_del_flips = 0;
  ...
  res.pq_flip_energy = curr_energy;
  res.pq_flip_iteration = itr; 

// queue 
  itr = 0;
  curr_energy = res.energies[0];
  past_energy = 0;
  UV_iparam = UV_ext;
  while(itr<max_iterations && std::abs(past_energy-curr_energy)>tol){
    std::cout << "Iteration q: " << itr+1 << std::endl;
    past_energy = curr_energy;
    //intrinsic flipping
    unsigned total_flips = 0;
    unsigned total_del_flips = 0;
  ...
  res.q_flip_energy = curr_energy;
  res.q_flip_iteration = itr; 
  ...
  while(random_runs--){
    ...
    UV_iparam = UV_ext;
    while(itr<max_iterations && std::abs(past_energy-curr_energy)>tol){
      std::cout << "Iteration r: " << itr+1 << std::endl;
      past_energy = curr_energy;
      //intrinsic flipping
      unsigned total_flips = 0;
      unsigned total_del_flips = 0;
    ...
    res.random_flip_energies.push_back(curr_energy);
    res.random_flip_iterations.push_back(itr);
