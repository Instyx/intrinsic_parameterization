...
Results optimize_single_new(Eigen::MatrixXd &V, Eigen::MatrixXi &F, EnergyType method, std::string dir, std::string mesh_name, const bool init_with_intrinsic, const bool priority_queue_flips){
  return optimize_single_new(V, F, method, dir, mesh_name, init_with_intrinsic, priority_queue_flips, -1);
}
...
Results optimize_single_new(Eigen::MatrixXd &V, Eigen::MatrixXi &F, EnergyType method, std::string dir, std::string mesh_name, const bool init_with_intrinsic, const bool priority_queue_flips, int random_runs){
  ...
  while(itr<max_iterations && std::abs(past_energy-curr_energy)>tol){
    ...
    if (priority_queue_flips) {
      ...
      total_flips = queue_flip(data_mesh, UV_iparam, total_del_flips, method, false);
    }
    end = std::chrono::high_resolution_clock::now();
  ...
  if(random_runs>0){
    while(random_runs--){
      ...
      while(itr<max_iterations && std::abs(past_energy-curr_energy)>tol){
        ...
        unsigned flips, del;
        start = std::chrono::high_resolution_clock::now();
        total_flips = queue_flip(data_mesh, UV_iparam, total_del_flips, method, true);
        end = std::chrono::high_resolution_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
        std::cout << "  found " << total_flips << " improving flips, " << total_del_flips <<" were delaunay in "<< duration << "ms" << std::endl;
        //res.flips.push_back(total_flips);
