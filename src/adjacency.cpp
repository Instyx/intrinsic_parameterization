...
int faceindex(const Eigen::Ref< const Eigen::Matrix<int,-1,3> > F,
              int f, int v)
{
    if (F(f,0) == v) return 0;
    if (F(f,1) == v) return 1;
...
int next_faceindex(const Eigen::Ref< const Eigen::Matrix<int,-1,3> > F,
                   int f, int v)
{
...
int prev_faceindex(const Eigen::Ref< const Eigen::Matrix<int,-1,3> > F,
                   int f, int v)
{
  int ret = faceindex(F,f,v);
  if (ret < 0) return -1;
...
void tt_adjacency(const Eigen::Ref< const Eigen::Matrix<int,-1,3> > F,
                  ...
                  Eigen::VectorXi & VVi)
{
    ...
    for (int i = 0; i < F.rows(); i++)
    {
        ...
        VV[vvd[F(i,1)]++] = F(i,2);
        VV[vvd[F(i,2)]++] = F(i,0);
        if (TT(i,0) < 0) VV[vvd[F(i,2)]++] = F(i,1);
        if (TT(i,1) < 0) VV[vvd[F(i,0)]++] = F(i,2);
        if (TT(i,2) < 0) VV[vvd[F(i,1)]++] = F(i,0);
    }
}
                ...
                )
{
    ...
    for (int i = 0; i < F.rows(); i++)
    {
        ...
        if (TT(i,0) < 0) VHE[vhed[F(i,2)]++] = HEHE(FHE(i,0));
        if (TT(i,1) < 0) VHE[vhed[F(i,0)]++] = HEHE(FHE(i,1));
        if (TT(i,2) < 0) VHE[vhed[F(i,1)]++] = HEHE(FHE(i,2));
    }
}
